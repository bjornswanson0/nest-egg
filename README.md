...
- **Paystub converter** — hand it a paystub PDF (or drop the file, paste its text, or type the five numbers) and it reads the pay frequency, gross, net, 401(k), and HSA lines entirely in your browser — no libraries, nothing uploaded. What they imply — annual income, true monthly take-home (payroll-deducted savings added back), 401(k) percentage, monthly HSA contribution — appears as ghost numbers on the matching questions, with the arithmetic on hover. Your answers are never filled in for you.
- **Statement inbox & tracking** — drop any document on the results page and it routes itself: Merrill and Schwab statements (PDF, CSV, or export) update the matching account balance and log a dated snapshot; paystubs route to the converter. Snapshots build a per-account history with deltas between drops, so your real balances accumulate a track record next to the projection. Balances are facts and write directly; plan choices only ever ghost-suggest.
...
- **Saved setups** — snapshot your whole configuration under a name and swap between setups from the results screen or the first question; stored on your device like everything else.
- **Navigable walkthrough** — the section rail jumps straight to any part of the questionnaire instead of forcing a linear march.
- **Built-in coaching** — savings rate vs. a needed-rate benchmark for your age and retirement goal (rebuilt from Money Guy's "How Much Should You Save?" table and verified against all 89 published cells), employer-match capture in dollars, the "one more 1%" compounding lesson, salary-multiple benchmarks by age, a **50/30/20 paycheck view**, and a checklist of concrete monthly moves.
